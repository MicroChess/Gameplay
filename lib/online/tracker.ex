defmodule Tracker do
    ...
    def handle_call(datapack, sender, state) do
        ...
        out = case Map.fetch(req, :type) do
            {:ok, "domove"  } -> DoMove.update_state(state, req)
            {:ok, "draw"    } -> Draw.update_state(state, req)
            {:ok, "resign"  } -> Resign.update_state(state, req)
            {:ok, "spectate"} -> {:ok, update_spectators(state, req.sender)}
