defmodule DoMove.Test do
    ...
    test "Game ok [correctly handles first three moves]" do
        move1 = %{ @white_req | from: {:e, 2}, to: {:e, 4}, count: 1}
        move2 = %{ @black_req | from: {:e, 7}, to: {:e, 5}, count: 1}
    ...
    test "Game ok [correctly rejects invalid black's first move]" do
        move = %{ @black_req | from: {:e, 7}, to: {:e, 5}, count: 1}
        assert {:error, _reason} = DoMove.update_state(@initial_state, move)
    end

    test "Game ok [correctly rejects white's ill-counted first move]" do
        move = %{ @white_req | from: {:e, 2}, to: {:e, 4}, count: 3}
        assert {:error, _reason} = DoMove.update_state(@initial_state, move)
    end

    test "Game ok [correctly rejects white's attempt to move black pieces]" do
